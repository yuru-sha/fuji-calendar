...
const QueueManager: React.FC = () => {
  ...
  const [backgroundJobs, setBackgroundJobs] =
    useState<BackgroundJobsStatus | null>(null);
  const [concurrencyInfo, setConcurrencyInfo] =
    useState<ConcurrencyInfo | null>(null);
  ...
  const updateConcurrency = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setConcurrencyMessage(
          `同時実行数を ${data.data.oldConcurrency} から ${data.data.newConcurrency} に変更しました`,
        );
  ...
  try {
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                ...
                <div className="mt-2 text-sm text-yellow-700">
                  ...
                  <p className="mt-1">
                    Redis を起動するか、Docker を使用してください：
                    <br />
                    <code className="bg-yellow-100 px-1 rounded text-xs">
                      docker run -d --name redis-fuji -p 6379:6379 redis:alpine
                    </code>
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewConcurrency(parseInt(e.target.value))
                    }
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    範囲: {concurrencyInfo.minConcurrency}-
                    {concurrencyInfo.maxConcurrency}
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={
                    loading ||
                    newConcurrency === concurrencyInfo.concurrency ||
                    !stats.enabled
                  }
              ...
              <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                ...
                <p>
                  •
                  同時実行数を減らすと、メモリ使用量が削減されますが処理速度が低下します
                </p>
                <p>
                  •
                  同時実行数を増やすと、処理速度が向上しますがメモリ使用量が増加します
                </p>
                <p>• 変更は即座に反映され、実行中のジョブには影響しません</p>
                <p>
                  • 15 分の天体計算に対応するため、stall タイムアウトは 20
                  分に設定済みです
                </p>
              </div>
            </div>
    ...
    setRenderError(
      `レンダリングエラー: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
