...
export class AuthServiceImpl implements AuthService {
  ...
  async authenticate(username: string, password: string, ipAddress?: string): Promise<{
    success: boolean;
    accessToken?: string;
    refreshToken?: string;
    message: string;
    ...
    try {
      ...
      if (!admin) {
        await this.authRepository.recordLoginAttempt(username, false, ipAddress);
        logger.warn('認証失敗 - ユーザーが見つかりません', { username, ipAddress });
        return {
          success: false,
      ...
      if (!isValidPassword) {
        await this.authRepository.recordLoginAttempt(username, false, ipAddress);
        logger.warn('認証失敗 - パスワードが正しくありません', { username, ipAddress });
        return {
          success: false,
          message: 'ユーザー名またはパスワードが正しくありません。'
        };
      ...
      await this.authRepository.saveRefreshToken(admin.id, refreshToken, refreshExpiresAt);
  ...
  async changePassword(adminId: number, currentPassword: string, newPassword: string): Promise<{
    success: boolean;
    message: string;
  }> {
    try {
  ...
  async verifyAccessToken(token: string): Promise<{
    ...
    try {
      ...
      logger.debug('アクセストークン検証失敗', { error: error instanceof Error ? error.message : error });
      return {
        valid: false
      };
    }
