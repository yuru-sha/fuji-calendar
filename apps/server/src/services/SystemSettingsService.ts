...
export class SystemSettingsService {
  ...
  async getSetting<T = any>(settingKey: string, defaultValue?: T): Promise<T> {
    ...
    throw new Error(`Setting not found and no default value provided: ${settingKey}`);
  }

  /**
   * 数値設定を取得
   */
  async getNumberSetting(settingKey: string, defaultValue?: number): Promise<number> {
   ...
   * 文字列設定を取得
   */
  async getStringSetting(settingKey: string, defaultValue?: string): Promise<string> {
   ...
   * 真偽値設定を取得
   */
  async getBooleanSetting(settingKey: string, defaultValue?: boolean): Promise<boolean> {
   ...
   * 設定値を更新
   */
  async updateSetting(settingKey: string, value: any, settingType?: string): Promise<void> {
    try {
   ...
   */
  async updateSettingAndReturn(settingKey: string, value: any, settingType?: string): Promise<any | null> {
    try {
      const updateData = this.prepareUpdateData(value, settingType);
  ...
  async getAstronomicalSettings(): Promise<{
    ...
    return {
      ...
      elevationTolerance: await this.getNumberSetting('elevation_tolerance', 0.05),
      searchInterval: await this.getNumberSetting('search_interval', 10),
      sunAngularDiameter: await this.getNumberSetting('sun_angular_diameter', 0.53),
      moonAngularDiameter: await this.getNumberSetting('moon_angular_diameter', 0.52)
    };
  }
}