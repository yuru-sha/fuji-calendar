...
function debugMoonRiseOnDate() {
  ...
  for (let hour = 18; hour <= 30; hour++) { // 30 = 翌日 6 時
    ...
    try {
      ...
      const moonPosition = Astronomy.Horizon(utcTime, observer, 'Moon', 'normal');
...
function calculateMoonrise() {
  ...
  try {
    const moonrise = Astronomy.SearchRiseSet('Moon', observer, 1, utcDate, 1);
    if (moonrise) {
      const jstTime = new Date(moonrise.getTime() + 9 * 60 * 60 * 1000);
      console.log(`月の出時刻: ${jstTime.toLocaleString('ja-JP', {timeZone: 'Asia/Tokyo'})}`);
      
      // 月の出時の方位角と高度
      const risePosition = Astronomy.Horizon(moonrise, observer, 'Moon', 'normal');
