...
# ヘルプ表示
show_help() {
    echo "ダイヤモンド富士・パール富士カレンダー Docker本番環境"
    echo ""
    echo "使用方法:"
    echo "  $0 [コマンド]"
    echo ""
...
backup_db() {
    ...
    mkdir -p "$BACKUP_DIR"
    docker-compose exec app sqlite3 /app/data/fuji-calendar.db ".backup /tmp/backup.db"
...
restore_db() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        docker-compose exec app sh -c "cp /tmp/restore.db /app/data/fuji-calendar.db"
        restart_prod
