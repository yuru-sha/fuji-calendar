...
// EventCacheService はキューシステムに移行済み
...
export class AdminController {
    ...
    try {
      ...
      if (locationData.latitude < -90 || locationData.latitude > 90) {
        return res.status(400).json({
          ...
          message: '緯度は-90 から 90 の間で指定してください。'
        });
      }
      ...
      if (locationData.longitude < -180 || locationData.longitude > 180) {
        return res.status(400).json({
          ...
          message: '経度は-180 から 180 の間で指定してください。'
        });
      }
      ...
      try {
        fujiAzimuth = astronomicalCalculator.calculateAzimuthToFuji(tempLocation);
      ...
      const newLocation = await prisma.location.create({
        data: {
          ...
          fujiAzimuth: Math.round(fujiAzimuth * 1000) / 1000,      // 自動計算（小数点 3 桁）
          fujiElevation: Math.round(fujiElevation * 1000000) / 1000000, // 自動計算（小数点 6 桁）
          fujiDistance: Math.round(fujiDistance * 100) / 100        // 自動計算（小数点 2 桁）
        }
      });
      ...
      res.status(201).json({
        success: true,
        data: newLocation,
        message: '撮影地点が正常に作成されました。イベント計算をバックグラウンドで実行中です。'
      });

      // 富士現象イベントの初期計算（非同期でバックグラウンド実行）
      this.scheduleBackgroundCalculation(newLocation.id, newLocation.name);
    } catch (error) {
  ...
  async updateLocation(req: Request, res: Response) {
    try {
      ...
      if (coordinatesChanged) {
        ...
        try {
          const fujiAzimuth = astronomicalCalculator.calculateAzimuthToFuji(tempLocation);
      ...
      res.json({
        ...
        message: fujiDataChanged 
          ? '撮影地点が正常に更新されました。イベント再計算をバックグラウンドで実行中です。'
          : '撮影地点が正常に更新されました。'
      });

      // 座標または富士山データが変更された場合、富士現象イベントを再計算（非同期実行）
      if (fujiDataChanged) {
        this.scheduleBackgroundRecalculation(id, updatedLocation.name);
      }
    } catch (error) {
  ...
  async deleteLocation(req: Request, res: Response) {
    try {
      ...
      // 関連する富士現象イベントも削除（Cascade 削除で自動削除されるが、ログ記録のため明示的に実行）
  ...
  async reverseGeocode(req: Request, res: Response) {
    try {
      ...
      const muniCd = result.muniCd || result.munuCd; // API 仕様変更に対応
      ...
      // 国土地理院 API が失敗した場合、簡易的な座標ベースの判定を使用
      try {
  ...
  private getSuggestedLocationName(lat: number, lng: number, prefecture: string): string {
    ...
    // 距離に応じて地名を調整
    if (minDistance < 0.05) { // 約 5km 以内
      return closestLandmark.name;
    } else if (minDistance < 0.1) { // 約 10km 以内
      return `${closestLandmark.name}周辺`;
    } else if (minDistance < 0.2) { // 約 20km 以内
  ...
  async pregenerateCache(req: Request, res: Response) {
    try {
      ...
      if (year < 2000 || year > 2100 || month < 1 || month > 12) {
        return res.status(400).json({
          error: 'Invalid range',
          message: '年は 2000-2100 年、月は 1-12 の範囲で指定してください。'
        });
      }
  ...
  async importLocations(req: Request, res: Response) {
    try {
      ...
      if (!Array.isArray(locations) || locations.length === 0) {
        return res.status(400).json({
          error: 'Invalid input',
          message: '有効な撮影地点データの配列を提供してください。'
        });
      }
      ...
      this.logger.info('撮影地点一括インポート開始', { count: locations.length });
        ...
        try {
          ...
          if (!locationData.name || !locationData.prefecture || 
            ...
            errors.push({
              index: i,
              data: locationData,
              error: '必須フィールドが不足しているか、形式が正しくありません。'
            });
            continue;
          }
          ...
          // 重複チェック（名前+都道府県の組み合わせ、または座標の近似一致）
          
          // 1. 同じ名前と都道府県の組み合わせをチェック
          ...
          if (existingByName) {
            errors.push({
              index: i,
              data: locationData,
              error: `同じ名前「${locationData.name}」の地点が${locationData.prefecture}に既に存在します。`
            });
            continue;
          }

          // 2. 座標の近似一致をチェック（100m 以内を重複とみなす）
          const coordinateTolerance = 0.001; // 約 100m 相当
          const existingByCoords = await prisma.location.findFirst({
            where: {
              latitude: {
                gte: locationData.latitude - coordinateTolerance,
                lte: locationData.latitude + coordinateTolerance
              },
              longitude: {
                gte: locationData.longitude - coordinateTolerance,
                lte: locationData.longitude + coordinateTolerance
              }
            }
          });
          ...
          if (existingByCoords) {
            errors.push({
              index: i,
              data: locationData,
              error: `座標が近い地点「${existingByCoords.name}」が既に存在します（100m 以内）。`
            });
            continue;
          }
          ...
          try {
            ...
            this.logger.warn('富士山への事前計算エラー（インポート時）', {
              locationName: locationData.name,
              index: i,
              error: calculationError instanceof Error ? calculationError.message : 'Unknown calculation error'
            });
          ...
          const newLocation = await prisma.location.create({
            data: {
              ...
              fujiElevation: Math.round(fujiElevation * 1000000) / 1000000,
      ...
      this.logger.info('撮影地点一括インポート完了', {
      ...
      res.status(201).json({
        ...
        message: `${results.length}件の撮影地点が正常にインポートされました。${errors.length}件のエラーがありました。イベント計算をバックグラウンドで実行中です。`
      });

    } catch (error) {
      this.logger.error('撮影地点一括インポートエラー', error, {
        requestId: (req as any).requestId
      });
      res.status(500).json({
        error: 'Internal server error',
        message: '撮影地点のインポートに失敗しました。'
      });
    }
  }
  ...
  /**
   * バックグラウンドでイベント計算をスケジュール
   */
  private scheduleBackgroundCalculation(locationId: number, locationName: string): void {
    // 非同期でバックグラウンド実行（エラーが発生してもメインフローには影響しない）
    this.performBackgroundCalculation(locationId, locationName).catch((error) => {
      this.logger.error('バックグラウンド計算エラー', {
        locationId,
        locationName,
        error: error instanceof Error ? error.message : 'Unknown error'
      });
  ...
  /**
   * 実際のバックグラウンド計算処理（新規作成時）
   */
  private async performBackgroundCalculation(locationId: number, locationName: string): Promise<void> {
    try {
      const currentYear = new Date().getFullYear();
      
      // 前年・当年・翌年の 3 年分のイベントキャッシュを生成
      const jobId = await queueService.scheduleLocationCalculation(
        locationId,
        currentYear - 1,
        currentYear + 1,
        'medium', // レスポンス優先のため priority を medium に変更
        `create-location-${locationId}-${Date.now()}`
      );
      
      this.logger.info('地点作成に伴う計算ジョブをバックグラウンドでスケジュール', {
        locationId,
        locationName,
        jobId,
      ...
      this.logger.error('バックグラウンド計算ジョブのスケジュール失敗', {
        locationId,
        locationName,
        error: error instanceof Error ? error.message : 'Unknown error'
      });
  ...
  /**
   * バックグラウンドでイベント再計算をスケジュール（更新時）
   */
  private scheduleBackgroundRecalculation(locationId: number, locationName: string): void {
    // 非同期でバックグラウンド実行（エラーが発生してもメインフローには影響しない）
    this.performBackgroundRecalculation(locationId, locationName).catch((error) => {
      this.logger.error('バックグラウンド再計算エラー', {
        locationId,
        locationName,
        error: error instanceof Error ? error.message : 'Unknown error'
      });
  ...
  /**
   * 実際のバックグラウンド再計算処理（更新時）
   */
  private async performBackgroundRecalculation(locationId: number, locationName: string): Promise<void> {
    try {
      const currentYear = new Date().getFullYear();
      ...
      const jobId = await queueService.scheduleLocationCalculation(
        ...
        currentYear + 1,
        'medium', // レスポンス優先のため priority を medium に変更
        `update-location-${locationId}-${Date.now()}`
      );
      
      this.logger.info('座標変更に伴う計算ジョブをバックグラウンドでスケジュール', {
        locationId,
        locationName,
        jobId,
        years: [currentYear - 1, currentYear, currentYear + 1]
      ...
      this.logger.error('バックグラウンド再計算ジョブのスケジュール失敗', {
        locationId,
        locationName,  
        error: error instanceof Error ? error.message : 'Unknown error'
      });
  ...
  private async performEventDeletion(locationId: number, locationName: string): Promise<void> {
    try {
      ...
      if (eventCount > 0) {
        // イベント削除（Cascade 削除で既に削除されている可能性があるが、明示的に実行）
