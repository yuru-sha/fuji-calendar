...
export class AstronomicalCalculatorImpl implements AstronomicalCalculator {
   ...
   * ダイアモンド富士を計算
   */
  async calculateDiamondFuji(date: Date, locations: Location[]): Promise<FujiEvent[]> {
    ...
    try {
      ...
      const eventPromises = locations.map(async (location) => {
        try {
          const events = await this.alignmentCalc.findDiamondFuji(date, location);
          return events;
        } catch (error) {
   ...
   * パール富士を計算
   */
  async calculatePearlFuji(date: Date, locations: Location[]): Promise<FujiEvent[]> {
    const startTime = Date.now();
    const allEvents: FujiEvent[] = [];
