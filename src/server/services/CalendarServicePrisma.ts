...
export class CalendarServicePrisma {
  ...
  async getMonthlyCalendar(year: number, month: number): Promise<CalendarResponse> {
    ...
    try {
      this.logger.info('月間カレンダー取得開始（Prismaベース）', { year, month });
      ...
      this.logger.info('月間カレンダー取得完了（Prismaベース）', {
        year,
        month,
        locationCount: locations.length,
      ...
      this.logger.error('月間カレンダー取得エラー（Prismaベース）', error, {
        year,
        month
      });
  ...
  async getDayEvents(dateString: string): Promise<EventsResponse> {
    ...
    try {
      this.logger.info('日次イベント取得開始（Prismaベース）', { dateString });
      ...
      const responseTime = Date.now() - startTime;
      this.logger.info('日次イベント取得完了（Prismaベース）', {
        dateString,
      ...
      this.logger.error('日次イベント取得エラー（Prismaベース）', error, { dateString });
   ...
   * LocationFujiEventをFujiEventに変換
   */
  private convertToFujiEvent(event: LocationFujiEvent, location: Location): FujiEvent {
    // EventTypeをFujiEventのtypeとsubTypeに変換
