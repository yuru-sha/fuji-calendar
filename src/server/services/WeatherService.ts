  ...
  async getWeatherInfo(
    ...
    try {
      ...
      if (diffDays === 0) {
        ...
        visibility = data.current.visibility / 1000; // mをkmに変換
      } else {
        // 未来の日付は日別予報
        const dayIndex = diffDays;
        weatherCode = data.daily.weather_code[dayIndex];
        cloudCover = data.daily.cloud_cover_mean[dayIndex];
        visibility = data.daily.visibility_mean[dayIndex] / 1000; // mをkmに変換
      }
      ...
      // 撮影条件を判定
      const recommendation = this.calculateRecommendation(condition, cloudCover, visibility);
