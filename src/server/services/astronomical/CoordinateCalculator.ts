...
export class CoordinateCalculator {
   ...
   * 撮影地点から富士山への方位角を計算
   */
  calculateAzimuthToFuji(location: { latitude: number; longitude: number }): number {
   ...
   */
  calculateDistanceToFuji(location: { latitude: number; longitude: number }): number {
    const lat1 = this.toRadians(location.latitude);
    const lon1 = this.toRadians(location.longitude);
    const lat2 = this.toRadians(FUJI_COORDINATES.latitude);
    const lon2 = this.toRadians(FUJI_COORDINATES.longitude);
   ...
   */
  calculateElevationToFujiSummit(
    ...
    // 地球曲率による見かけの高度低下
    const curvatureDrop = Math.pow(distance, 2) / (2 * CoordinateCalculator.EARTH_RADIUS);
